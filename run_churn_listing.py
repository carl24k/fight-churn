...
def sql_listing(chapter, listing, name, schema, mode, param_dict, insert=False, save_ext=None):
    ...
    with open('fightchurn/listings/chap%d/%s.sql' % (chapter, _full_listing_name(chapter, listing, name, insert)), 'r') as myfile:
...
def load_and_check_listing_params(args):
    ...
    conf_path='fightchurn/listings/conf/%s_listings.json' % schema
