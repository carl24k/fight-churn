...
def cohort_plot(data_set_path, metric_to_plot='',ncohort=10):
    ...
    groups = pd.qcut(purchase_data[metric_to_plot], ncohort, duplicates='drop')
    cohort_means = purchase_data.groupby(groups)[metric_to_plot].mean()
    cohort_purchases = purchase_data.groupby(groups)['purchase'].mean()
    plot_frame = pd.DataFrame({metric_to_plot: cohort_means.values, 'purchase_rate': cohort_purchases.values})
